...
class CategoryModelTest(TestCase):
    ...
    def test_soft_delete(self):
        ...
        self.assertFalse(Category.objects.filter(id=category_id, is_deleted=False).exists())
...
class ProductModelTest(TestCase):
    ...
    def test_sku_generation(self):
        ...
        self.assertEqual(len(product.sku.split('-')), 4)  # 4 partes separadas por guiones
    
    def test_product_str_representation(self):
        """Test para la representación string del producto."""
        product = Product.objects.create(**self.product_data)
        expected_str = f"{product.name} - {product.get_size_display()} - {product.get_color_display()}"
    ...
    def test_profit_margin_calculation(self):
        ...
        expected_margin = ((Decimal('200.00') - Decimal('120.00')) / Decimal('120.00')) * 100
...
class ProductImageModelTest(TestCase):
    ...
    def setUp(self):
        ...
        self.category = Category.objects.create(
            name='Vestidos',
            description='Vestidos para niñas'
        )
