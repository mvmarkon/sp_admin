...
class ProductStockUpdateSerializer(serializers.Serializer):
    ...
    quantity = serializers.IntegerField(
        min_value=1, help_text="Cantidad a agregar o quitar del stock"
    )
    ...
    reason = serializers.CharField(
        max_length=200, required=False, help_text="Razón del ajuste de stock (opcional)"
    )
...
class ProductBulkUpdateSerializer(serializers.Serializer):
    ...
    updates = serializers.DictField(help_text="Diccionario con los campos a actualizar")
...
class ProductSearchSerializer(serializers.Serializer):
    ...
    search = serializers.CharField(
        required=False, help_text="Término de búsqueda (nombre, SKU, descripción)"
    )
    category = serializers.IntegerField(required=False, help_text="ID de la categoría")
    size = serializers.ChoiceField(
        choices=Product.SIZES, required=False, help_text="Talla del producto"
    )
    color = serializers.ChoiceField(
        choices=Product.COLORS, required=False, help_text="Color del producto"
    )
    min_price = serializers.DecimalField(
        max_digits=10, decimal_places=2, required=False, help_text="Precio mínimo"
    )
    max_price = serializers.DecimalField(
        max_digits=10, decimal_places=2, required=False, help_text="Precio máximo"
    )
    in_stock = serializers.BooleanField(
        required=False, help_text="Filtrar solo productos con stock"
    )
    low_stock = serializers.BooleanField(
        required=False, help_text="Filtrar solo productos con stock bajo"
    )
