    ...
    def has_permission(self, request, view):
        ...
        if request.method in ['POST', 'PUT', 'PATCH']:
            return (
                ...
                request.user.has_perm('products.change_product') or
                request.user.has_perm('products.add_product')
            )
...
class StockUpdatePermission(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        return (
            request.user.is_staff or
            request.user.has_perm('products.change_product') or
            request.user.groups.filter(name='Inventory Managers').exists()
        )
...
class CategoryPermission(permissions.BasePermission):
    ...
    def has_permission(self, request, view):
        ...
        return (
            request.user.is_staff or
            request.user.has_perm('products.add_category') or
    ...
    def has_object_permission(self, request, view, obj):
        ...
        if request.method == 'DELETE':
            return (
                request.user.is_superuser or
                request.user.has_perm('products.delete_category')
            )
