...
class ProductFilter(django_filters.FilterSet):
    ...
    price_range = django_filters.RangeFilter(
        field_name='price',
        help_text="Rango de precios (ej: 100,500)"
    )
    ...
    min_stock = django_filters.NumberFilter(
        field_name='stock',
        lookup_expr='gte',
        help_text="Stock mínimo"
    )
    
    max_stock = django_filters.NumberFilter(
        field_name='stock',
        lookup_expr='lte',
        help_text="Stock máximo"
    )
    ...
    low_stock = django_filters.BooleanFilter(
        ...
        help_text="Filtrar productos con stock bajo"
    )
    
    out_of_stock = django_filters.BooleanFilter(
        method='filter_out_of_stock',
        help_text="Filtrar productos agotados"
    )
    ...
    sizes = django_filters.BaseInFilter(
        field_name='size',
        help_text="Múltiples tallas separadas por coma (ej: S,M,L)"
    )
...
class CategoryFilter(django_filters.FilterSet):
    ...
    is_active = django_filters.BooleanFilter(
        field_name='is_active',
        help_text="Filtrar categorías activas/inactivas"
    )
