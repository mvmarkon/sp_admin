...
class ProductListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        show_inactive = (
            self.request.query_params.get("show_inactive", "false").lower() == "true"
        )
        if self.request.method == "GET":
...
class ProductDetailView(generics.RetrieveUpdateDestroyAPIView):
    ...
    def get_queryset(self):
        queryset = super().get_queryset()
        return queryset.select_related("category").prefetch_related("additional_images")
...
def update_product_stock(request, sku):
    ...
    if serializer.is_valid():
        ...
        if success:
            ...
            message = (
                f"Stock {action}: {quantity} unidades. "
            ...
            return Response(
                {"message": message, "product": product_serializer.data},
...
def low_stock_products(request):
    ...
    """
    products = Product.objects.filter(is_deleted=False, is_active=True).select_related(
        "category"
    )
    ...
    return Response(
        {"count": len(low_stock_products), "products": serializer.data},
        status=status.HTTP_200_OK,
    )
...
def out_of_stock_products(request):
    ...
    products = Product.objects.filter(
        stock=0, is_deleted=False, is_active=True
    ).select_related("category")

    serializer = ProductListSerializer(products, many=True)

    return Response(
        {"count": products.count(), "products": serializer.data},
        status=status.HTTP_200_OK,
    )
    ...
    return Response(
        {
            ...
            "stock_alerts": {
                ...
                "out_of_stock_percentage": (
                    (out_of_stock_count / total_products * 100)
                    if total_products > 0
                    else 0
                ),
            },
...
def search_products(request):
    ...
    return Response(
        {"query": query, "count": products.count(), "products": serializer.data},
        status=status.HTTP_200_OK,
    )
